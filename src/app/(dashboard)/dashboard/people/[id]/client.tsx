...
import { getEmployeeById, type Employee } from "@/lib/mock-data";
...
export default function EmployeeProfileClient() {
  ...
  useEffect(() => {
    const data = getEmployeeById(params.id as string);
    setEmployee(data);
    setLoading(false);
  }, [params.id]);
      ...
      {activeTab === "overview" && (
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          {employee.reports.length > 0 && (
            <Card>
              <CardHeader><CardTitle>Direct Reports ({employee.reports.length})</CardTitle></CardHeader>
              <CardBody className="p-0">
                <div className="divide-y divide-slate-100 dark:divide-slate-800">
                  {employee.reports.map((r) => (
                    <Link key={r.id} href={`/dashboard/people/${r.id}`} className="flex items-center gap-2.5 px-5 py-2.5 hover:bg-slate-50 dark:hover:bg-slate-800/50">
